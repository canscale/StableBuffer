...
do {
  ...
  assert (e.elements.size() == 3);
  B.add(e, 3);
  assert (e.elements.size() == 5);
};
...
let clearSuite = suite(
  "clear",
  [
    test(
      "before clearing, buffer has at least one element",
      B.capacity(buffer) > 0 and B.size(buffer) > 0,
      M.equals(T.bool(true)),
    ),
    test(
      "clears a buffer with elements",
      do {
        B.clear(buffer);
    ...
    test(
      "clearing an empty buffer makes no difference",
      do {
